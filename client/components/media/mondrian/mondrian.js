...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.Divisible = undefined;

	var _react = __webpack_require__(1);

	var _react2 = _interopRequireDefault(_react);

	var _Mondrian = __webpack_require__(2);

	var _Mondrian2 = _interopRequireDefault(_Mondrian);

	var _clickcoord = __webpack_require__(9);

	var _clickcoord2 = _interopRequireDefault(_clickcoord);
	...
	var Divisible = exports.Divisible = _react2.default.createClass({
	  displayName: "Divisible",
	  componentWillMount: function componentWillMount() {
	    this.core = new _Mondrian2.default(this.props.configuration || defaultConfig);

	    this.state = {
	      cells: this.core.state.getIn(['structure', 'data']),
	      root: this.core.state.getIn(['structure', 'data', 0, 'value']),
	      offset: this.core.state.getIn(['offset'])
	    };
	  },
	  render: function render() {
	    return _react2.default.createElement(
	      'svg',
	      { id: 'mondrian', viewBox: '0 0 ' + (this.state.root.getIn(['dimensions', 'height']) + 25) + ' ' + (this.state.root.getIn(['dimensions', 'width']) + 25), height: 300, width: 300 },
	      _react2.default.createElement(
	        'g',
	        null,
	        _react2.default.createElement('rect', {
	          height: this.state.root.getIn(['dimensions', 'height']),
	          width: this.state.root.getIn(['dimensions', 'width']),
	          x: this.state.root.getIn(['coords', 'x']) + this.state.offset.get('x'),
	          y: this.state.root.getIn(['coords', 'y']) + this.state.offset.get('y'),
	          style: { stroke: 'black', strokeWidth: 3 }
	        }),
	        this.state.cells.map(function (cell) {
	          if (cell && cell.get && cell.get('shouldRender') !== false) {
	            var styles = this.generateStyles(cell);
	            return this.renderCell(cell, styles);
	          }
	        }, this)
	      )
	    );
	  },
	  divide: function divide(cell, direction) {
	    this.core.createChildren(cell, direction);
	  ...
	  generateStyles: function generateStyles(cell) {
	    var returned = {
	      animationDuration: '0.4s',
	      animationName: 'rect_' + cell.get('__l') + '_' + cell.get('__n'),
	      animationFillMode: 'forwards',
	      height: cell.getIn(['value', 'dimensions', 'height']),
	      width: cell.getIn(['value', 'dimensions', 'width']),
	      x: cell.getIn(['value', 'coords', 'x']),
	      y: cell.getIn(['value', 'coords', 'y']),
	      position: 'relative',
	      zIndex: cell.get('__l')
	    };
	    return returned;
	  },
	  setAnimationDirection: function setAnimationDirection(topRight, bottomLeft) {
	  ...
	  mapToRect: function mapToRect(coords, rect) {
	  ...
	  onClick: function onClick(divide, cell, event) {
	    if (cell.getIn(['value', 'canDivide'])) {
	      divide(cell, this.mapToRect((0, _clickcoord2.default)(event), event.target));
	    }
	  },
	  renderCell: function renderCell(cell, styles) {
	    return _react2.default.createElement("rect", {
	      className: 'rectangle ',
	      onClick: this.onClick.bind(this, this.divide, cell),
	      style: styles,
	      fill: cell.getIn(['value', 'color']),
	      stroke: "black",
	      strokeWidth: "4",
	      key: cell.get('__l') + '' + cell.get('__n'),
	      ref: cell.get('__l') + '' + cell.get('__n')
	    });
	  }
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _basicTree = __webpack_require__(3);

	var _basicTree2 = _interopRequireDefault(_basicTree);

	var _randomSet = __webpack_require__(4);
	...
	var _cssanimation = __webpack_require__(8);
	...
	var Mondrian = function () {
	  ...
	  _createClass(Mondrian, [{
	    key: 'updateState',
	    value: function updateState(state, newState) {
	      state = state.mergeDeep(newState);
	      return state;
	    }
	  }, {
	    key: 'makeStructure',
	    value: function makeStructure(state) {
	      this.structure = new _basicTree2.default(state.get('structure'));
	      return this.structure.state;
	    }
	  }, {
	    key: 'makeColor',
	    value: function makeColor(state) {
	      this.color = new _color2.default(state.get('color'));
	      return;
	    }
	  }, {
	    key: 'createChildren',
	    value: function createChildren(cell, quadrant) {
	      var quad = this.determineDirection(cell.getIn(['value', 'dimensions']), quadrant);
	      this.structure.goToNode(cell);
	      this.structure.node = this.structure.node.set('canDivide', false);
	      if (quad.get('x') !== 0 || quad.get('y') !== 0) {
	        var cells = this.divideIntoCells(this.structure.node, quad);
	        this.structure.children = cells;

	        this.structure.childrenItems.forEach(function (cell) {
	          if (cell && cell.getIn(['value', 'animation'])) {
	            var animation = new _cssanimation2.default('rect_' + cell.get('__l') + '_' + cell.get('__n'), cell.getIn(['value', 'animation', 0]).toJS(), cell.getIn(['value', 'animation', 1]).toJS());
	      ...
	      this.state = this.state.set('structure', this.structure.state);
	      return this.state;
	    }
	  }, {
	    key: 'determineDirection',
	    value: function determineDirection(dimensions, quadrant) {
	      var quad = (0, _immutable.Map)(quadrant);

	      quad = this.constrictTransform(dimensions, quad);
	      quad = this.limitRepetition(quad);
	      quad = this.countTransform(quad);

	      return quad;
	    }
	  }, {
	    key: 'limitRepetition',
	    value: function limitRepetition(quad) {
	      var q = quad,
	          isX = Math.abs(q.get('x')) > 0,
	          direction = isX ? 'x' : 'y',
	          opposite = isX ? 'y' : 'x';
	      if (this.state.getIn(['counter', direction]) > 2) {
	    ...
	    key: 'countTransform',
	    value: function countTransform(quad) {
	      var q = quad,
	          times = this.state.get('counter'),
	          isX = Math.abs(q.get('x')) > 0,
	          counted = isX ? 'x' : 'y',
	          zeroed = isX ? 'y' : 'x';

	      times = times.set(counted, times.get(counted) + 1);
	      times = times.set(zeroed, 0);
	      this.state = this.state.set('counter', times);
	      return q;
	    }
	  }, {
	    key: 'constrictTransform',
	    value: function constrictTransform(dimensions, quad) {
	      ...
	      return _definitions.xy;
	    }
	  }, {
	    key: 'divideIntoCells',
	    value: function divideIntoCells(cell, direction) {
	    ...
	    key: 'addColors',
	    value: function addColors(cells, parent) {
	      var _this = this;

	      cells = cells.map(function (item, index) {
	        if (index == cells.size - 1) {
	          return item.set('color', parent.get('color'));
	        } else if (Math.random() > 0.5) {
	          return item.set('color', _this.color.rybRandom().rgba());
	        } else {
	          return item;
	        }
	      });
	    ...
	    key: 'divideCell',
	    value: function divideCell(cell, coord, dimension, order) {
	      var _this2 = this;
	      ...
	      variation = new _randomSet.ratio({
	        attempts: this.structure.branches,
	        total: parent.getIn(['dimensions', dimension]),
	        variation: this.state.get('mutation'),
	        round: this.state.get('pixel')
	      });
	      variation.determineAll();
	      variation.chosen.forEach(function (choice, index) {
	        reduction = variation.chosen.takeUntil(function (value, i) {
	          return i >= index;
	        });
	        reduction = reduction.reduce(function (a, b) {
	          return a + b;
	        }, 0);
	        child = _this2.makeCell(parent);
	        child = child.updateIn(['coords', coord], function (c) {
	          return c += reduction || 0;
	        });
	    ...
	    key: 'addAnimCoords',
	    value: function addAnimCoords(cells, parent) {
	      var cell = cells.last(),
	          returned = cells.pop();
	      cell = cell.set('animation', (0, _immutable.fromJS)([{ x: parent.getIn(['coords', 'x']) + 'px;',
	    ...
	    key: 'makeCell',
	    value: function makeCell(cell) {
	      var returned = cell,
	          modX = this.state.getIn(['offset', 'x']),
	          modY = this.state.getIn(['offset', 'y']);
	      returned = returned.delete('animation'), returned = returned.set('canDivide', true), returned = returned.updateIn(['coords', 'x'], function (x) {
	        return x += modX;
	      }), returned = returned.updateIn(['coords', 'y'], function (y) {
	        return y += modX;
	      });
	      return returned;
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.ratio = exports.set = undefined;
	...
	var randomSet = function (_Tree) {
	  ...
	  function randomSet() {
	    ...
	    var setSize = args.set && args.set.constructor == Array ? args.set.length : false;
	    var max = setSize ? setSize : args.attempts;

	    var _this = _possibleConstructorReturn(this, Object.getPrototypeOf(randomSet).call(this, { config: { branches: max, depth: 1 } }));

	    _this._max = max;
	    _this._spread = (0, _immutable.Map)();
	    _this._spread = _this.setDefaults(_this._spread, args);
	    _this._spread = _this.makeChoices(_this._spread);
	    return _this;
	  }

	  _createClass(randomSet, [{
	    key: 'setDefaults',
	    value: function setDefaults(spread, args) {
	      spread = spread.set('attempts', args.attempts);
	      spread = spread.set('originals', (0, _immutable.List)(args.set));
	      return spread;
	    }
	  }, {
	    key: 'makeChoices',
	    value: function makeChoices(spread) {
	      var originals = this.makeListFrom(spread);
	      this.root = (0, _immutable.List)();
	    ...
	    key: 'makeListFrom',
	    value: function makeListFrom(spread) {
	    ...
	    key: 'createListExNihlo',
	    value: function createListExNihlo(length) {
	      var newSpread = (0, _immutable.List)();
	    ...
	    value: function determine() {
	      var seed = Math.floor(Math.random() * this._spread.get('possibilities').size),
	          val = void 0;
	    ...
	    key: 'removePossibility',
	    value: function removePossibility(spread, index) {
	      spread = spread.update('possibilities', function (item) {
	        return item.filterNot(function (i) {
	          return i === index;
	        });
	      });
	      return spread;
	    ...
	    key: 'toRoot',
	    value: function toRoot(value) {
	      var chosen = this.root;
	...
	var variedRatio = function (_randomSet) {
	  ...
	  function variedRatio() {
	    _classCallCheck(this, variedRatio);

	    return _possibleConstructorReturn(this, Object.getPrototypeOf(variedRatio).apply(this, arguments));
	  }
	  ...
	  _createClass(variedRatio, [{
	    ...
	    value: function setDefaults(spread, args) {
	      ...
	      var type = spread.get('variation') ? 'varied' : 'uniform';
	    ...
	    key: 'createRoundedList',
	    value: function createRoundedList(spread) {
	      var maxVariation = spread.get('variation'),
	      ...
	      for (var i = 0; i < this.attempts; i++) {
	        var _seed = this.roundNumTo(Number(Math.random() * (adjVariation + adjVariation) - adjVariation.toFixed(0)), round);
	        var variation = Number((part + _seed).toFixed(0));
	      ...
	      var _loop = function _loop() {
	        ...
	        newSpread = newSpread.map(function (item, index) {
	          if (absSeed == 0) {
	            return item;
	          }
	          if (dist !== 0) {
	    ...
	    key: 'createUniformList',
	    value: function createUniformList(spread) {
	      var part = spread.get('total') / this.attempts,
	          newSpread = (0, _immutable.List)();
	      for (var i = 0; i < this.attempts; i++) {
	...
	var set = exports.set = randomSet;
	var ratio = exports.ratio = variedRatio;
...
/***/ function(module, exports) {
	...
	var Color = function () {
		...
		_createClass(Color, [{
			...
			key: "rgbaInvert",
			value: function rgbaInvert(percent) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.defaults = exports.initial = exports.cell = exports.cd = exports.wh = exports.xy = undefined;

	var _immutable = __webpack_require__(5);

	var xy = exports.xy = (0, _immutable.Map)({ x: 0, y: 0 });
	var wh = exports.wh = (0, _immutable.Map)({ width: 0, height: 0 });
	var cd = exports.cd = (0, _immutable.Map)({
	  coords: xy,
	  dimensions: wh
	});
	var cell = exports.cell = (0, _immutable.Map)({
	...
	var initial = exports.initial = (0, _immutable.Map)({
	  value: (0, _immutable.Map)({
	    canDivide: true,
	    coords: (0, _immutable.Map)({ x: 170, y: 15 }),
	    dimensions: (0, _immutable.Map)({ width: 600, height: 600 }),
	    color: 'rgba(255,255,255,1)'
	  })
	});
	var defaults = exports.defaults = (0, _immutable.Map)({
	  pixel: 15,
	  offset: xy.set('y', -1).set('x', -1),
	  mutation: 0,
	  counter: xy,
	  color: _immutable.List.of(255, 255, 255, 1),
	  structure: (0, _immutable.Map)({
	    config: (0, _immutable.Map)({
	      depth: 5,
	      branches: 2
	    }),
	    data: _immutable.List.of(initial)
	  })
	});
...
/***/ function(module, exports) {
	...
	var cssAnimationMaker = function () {
	    function cssAnimationMaker(name) {
	        _classCallCheck(this, cssAnimationMaker);
	    ...
	    _createClass(cssAnimationMaker, [{
	        key: 'printRulesAt',
	        value: function printRulesAt(index) {
	            var returned = '',
	        ...
	        key: 'makeKeyframesRule',
	        value: function makeKeyframesRule(sheet) {
	            try {
	    ...
	    return cssAnimationMaker;
	}();

	exports.default = cssAnimationMaker;
